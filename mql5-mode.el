;; filename : mql5-mode.el

;;;###autoload
(require 'cl-lib)
(require 'company)

(defvar mql5-mode-constants
  '("ACCOUNT_ASSETS"
    "ACCOUNT_BALANCE"
...
(defvar mql5-mode-indicator-settings
  '("indicator_color"
    "indicator_label"
    "indicator_level"
    "indicator_style"
...
(defvar mql5-mode-indicator-settings-regex
  "\\<\\(indicator_color[0-9]+\\|indicator_label[0-9]+\\|indicator_level[0-9]+\\|indicator_style[0-9]+\\|indicator_type[0-9]+\\|indicator_width[0-9]+\\)\\>")

(defvar mql5-mode-builtins
  '("AccountInfoDouble"
    "AccountInfoInteger"
    "AccountInfoString"
    "Alert"
...
(defvar mql5-mode-defined-variables
  '("DXVertexLayout"
    "MqlBookInfo"
    "MqlCalendarCountry"
    "MqlCalendarEvent"
...
(defvar mql5-mode-reserved-words
  '("__DATETIME__"
    "__DATE__"
    "__FILE__"
    "__FUNCSIG__"
...
(defun company-mql5-backend (command &optional arg &rest ignored)
  (interactive (list 'interactive))

  (cl-case command
    (interactive (company-begin-backend 'company-mql5-backend))
    (prefix (and (eq major-mode 'mql5-mode)
                 (company-grab-symbol)))
    (candidates
     (cl-remove-if-not
      (lambda (c) (string-prefix-p arg c))
      (append mql5-mode-constants
              mql5-mode-indicator-settings
              mql5-mode-builtins
              mql5-mode-defined-variables
              mql5-mode-reserved-words
              mql5-mode-indicators)))))

(define-derived-mode mql5-mode c++-mode "MQL5" "Major Mode for MQL5 Language"
  (set (make-local-variable 'font-lock-multiline) t)
  (font-lock-add-keywords
   nil
   `((,(regexp-opt mql5-mode-constants 'words) . font-lock-constant-face)
     (,mql5-mode-indicator-settings-regex . font-lock-constant-face)
     (,(regexp-opt mql5-mode-builtins 'words) . font-lock-builtin-face)
     (,(regexp-opt mql5-mode-defined-variables 'words) . font-lock-doc-face)
     (,(regexp-opt mql5-mode-reserved-words 'words) . font-lock-keyword-face)
     (,(regexp-opt mql5-mode-indicators 'words) . font-lock-warning-face))
   )
  )

(add-hook 'mql5-mode-hook (lambda () (add-to-list 'company-backends 'company-mql5-backend)))

(add-to-list 'auto-mode-alist '("\\.mq5$" . mql5-mode))
(add-to-list 'auto-mode-alist '("\\.mqh$" . mql5-mode))
